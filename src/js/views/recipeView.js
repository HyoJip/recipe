...
const formatCount = (count) => {
	const newCount = count.toFixed(1);
	const [int, dec] = newCount.split(".").map((el) => parseInt(el));
...
export const renderRecipe = (recipe, isLiked) => {
	const markup = `
        <figure class="recipe__fig">
            <img src="${recipe.image}" alt="${recipe.title} image" class="recipe__img">
...
export const updateServingsIngredients = (recipe) => {
	...
	// Update Ingredients
	const counts = document.querySelectorAll(".recipe__count");
	Array.from(counts).forEach((el, i) => {
